...
/// is mid-transition (e.g. dialog pop, Flushbar auto-dismiss race).
Future<void> showFlushBar(BuildContext context,
    {required String message, String? title}) async {
  // Try the navigator key's overlay first (always available), then fallback
  // to Overlay.of(context). This avoids "No Overlay widget found" when the
  // context comes from the navigator itself (e.g. navKey.currentContext).
  OverlayState? overlay;
  try {
    overlay = navKey.currentState?.overlay;
  } catch (_) {}
  overlay ??= Overlay.maybeOf(context);
  if (overlay == null) return; // Nothing to show on – skip silently.
