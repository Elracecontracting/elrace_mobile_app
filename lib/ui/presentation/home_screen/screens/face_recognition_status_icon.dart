...
class FaceRecognitionStatusIcon extends StatefulWidget {
  ...
  const FaceRecognitionStatusIcon({Key? key, required this.status}) : super(key: key);

  @override
  State<FaceRecognitionStatusIcon> createState() => _FaceRecognitionStatusIconState();
}

class _FaceRecognitionStatusIconState extends State<FaceRecognitionStatusIcon> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(covariant FaceRecognitionStatusIcon oldWidget) {
    ...
    if (widget.status == FaceRecognitionStatus.matching) {
      _controller.repeat();
    } else if (widget.status != FaceRecognitionStatus.idle && oldWidget.status == FaceRecognitionStatus.idle) {
  ...
  Widget build(BuildContext context) {
    if (widget.status == FaceRecognitionStatus.idle) return const SizedBox.shrink();
    if (widget.status == FaceRecognitionStatus.matching) {
      return Column(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('*** Blink both eyes once clearly for detection',textAlign: TextAlign.center,)
              ],
            ),
          ),
    ...
    return ScaleTransition(
      scale: _scaleAnimation,
      child: Icon(
        widget.status == FaceRecognitionStatus.matched ? Icons.check_circle : Icons.close,
        color: widget.status == FaceRecognitionStatus.matched ? Colors.green : Colors.red,
        size: 50,
      ),
    );
  }
