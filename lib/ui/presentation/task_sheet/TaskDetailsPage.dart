...
class TaskDetailsPage extends StatefulWidget {
  ...
  const TaskDetailsPage({Key? key, required this.loginResponseModel, required this.taskId, required this.project_id}) : super(key: key);
...
class _TaskDetailsPageState extends State<TaskDetailsPage> {
  ...
  Future<void> _fetchTimesheetData() async {
    final url = Uri.parse("https://test.elrace.com/api/count/timesheets/by/days");
      ...
      "params": {
        ...
        "date_list": List.generate(
          selectedEndDate.difference(selectedStartDate).inDays + 1,
              (i) => DateFormat('yyyy-MM-dd').format(selectedStartDate.add(Duration(days: i))),
        ),
      }
    };
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          timesheetData = List<Map<String, dynamic>>.from(data['result']['timesheets']);
        });
      } else {
                  ...
                  child: GestureDetector(
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => EmptyShiftPage(
                            ...
                            selectedDate: date!,
                            taskId: widget.taskId,
                            project_id: widget.project_id,// 👈 Ensure 'date' is parsed correctly earlier in your loop
                          ),
                        ),
                      );
                    },
                    child: Container(
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: Colors.grey.withAlpha((0.2 * 255).toInt()), blurRadius: 6, spreadRadius: 1)],
                        gradient: const LinearGradient(colors: [Colors.white, Colors.grey], begin: Alignment.topCenter, end: Alignment.bottomRight),
                      ),
                      child: Padding(
                          ...
                          children: [
                            Container(
                              ...
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Text(day, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black)),
                                  Text(weekday, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: appFontColor)),
                                ],
                              ),
                            ),
                            Container(
                            ...
                            Expanded(
                              child: Padding(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    _buildStatusLabel("In Progress (${item['inprogress']})", const Color(0xFFBA1719)),
                                    _buildStatusLabel("Submitted (${item['submitted']})", Colors.blue),
                                    _buildStatusLabel("Approved (${item['approved']})", const Color(0xFF00D17A)),
                                  ],
                                ),
                              ),
                            ),
  ...
  Widget _buildDateContainer(String text) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withAlpha((0.2 * 255).toInt()), blurRadius: 6, spreadRadius: 2, offset: const Offset(0, 4))],
      ),
      child: Text(
      text,
